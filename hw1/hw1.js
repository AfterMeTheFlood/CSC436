...
const Product = class Product {
  constructor(param) {
    ...
    this.name = params[0];
    this.price = Number(params[1]);
    this.availability = params[2];
  }
  /**
   * *****************
   * ***** TODO ******
  ...
  static halfOff = (products) => {
    let halfOffProducts = [];
    products.forEach((product) => {
      let halfOffProduct = { ...product };
      halfOffProduct.price = halfOffProduct.price * 0.5;
      halfOffProducts.push(halfOffProduct);
    });
  ...
  static printProducts = (products) => {
    products.forEach((product) => {
      ...
      console.log(
        `Product: ${product.name}, Cost: ${price}, Availability: ${availability}`
      );
    });
  };
};
